...
本vnpy接口：https://github.com/tianqin18/vnpy-tianqin
...
########################################################################
class TianQinGateway(VtGateway):
    """天勤行情接口"""

    # ----------------------------------------------------------------------
    def __init__(self, eventEngine, gatewayName='TianQin'):
        """Constructor"""
        super(TianQinGateway, self).__init__(eventEngine, gatewayName)
        self.data = {}
        self.client = None
        self.requests = []
    ...
    def subscribe_chart(self, ins_id, duration_seconds, data_length=200, callback_func=None):
        ...
        if data_length > 8964:
            data_length = 8964
        if callback_func:
            self.chart_callback_func[chart_id] = callback_func
        req = {
            "aid": "set_chart",
            "chart_id": chart_id,
            "ins_list": ins_id,
    ...
    def _process_pack(self, event):
        ...
        for data in datas:
            self.data = dict_merge(self.data, data)
